...
def get_PaSST(pretrained=False, ckpt_path='None', n_classes=10, in_channels=1, fstride=10, tstride=10,
              input_fdim=128, input_tdim=101, u_patchout=0, s_patchout_t=0, s_patchout_f=0,
              ):
    """
    :param arch: Base ViT or Deit architecture
    :param pretrained: use pretrained model on imagenet
    ...
    if pretrained:
        ckpt_path = '/SATA02/chenxy/Code/Self-distillation/DCASE2025/my_dcase2025/ckpt/passt-s-f128-p16-s10-ap.476-swa.pt'
...
if __name__ == '__main__':
    input = torch.randn(1, 1, 128, 101)
    model1 = get_model(pretrained=False)
    # model2 = get_PaSST(pretrained=True)
    ...
    output2 = model1(input)

    print(output1[0].size(), output2[0].size())
