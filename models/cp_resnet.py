...
class SD_Network(nn.Module):
    def __init__(self, config):
        ...
        self.attention1 = nn.Sequential(
            SepConv(
                channel_in=n_channels[0],
                channel_out=n_channels[0]
            ),
            nn.BatchNorm2d(n_channels[0]),
            nn.ReLU(),
        ...
        self.attention3 = nn.Sequential(
            SepConv(
                channel_in=n_channels[1],
                channel_out=n_channels[1]
            ),
    ...
    def forward(self, x):
        ...
        out4 = self.feed_forward(out_feature_list[0]).squeeze(2).squeeze(2)
        out3 = self.feed_forward(out_feature_list[1]).squeeze(2).squeeze(2)
        out2 = self.feed_forward(out_feature_list[2]).squeeze(2).squeeze(2)
        out1 = self.feed_forward(out_feature_list[3]).squeeze(2).squeeze(2)
...
def get_cp_resnet(rho=8, in_channels=1, arch="cp_resnet", n_classes=10,
              base_channels=32, cut_channels_s2=0, cut_channels_s3=36, channels_multiplier=2, n_blocks=(2, 1, 1),
              s1_group=1, s2_group=2, s3_group=1,
              maxpool_pos_stage1=None, maxpool_pos_stage2=None, maxpool_pos_stage3=None,
              maxpool_stage1_pos0_kernel=(2, 2), maxpool_stage1_pos0_stride=(2, 2),
...
def get_cp_resnet_sd(rho=8, in_channels=1, arch="cp_resnet", n_classes=10,
              base_channels=32, cut_channels_s2=0, cut_channels_s3=36, channels_multiplier=2, n_blocks=(2, 1, 1),
              s1_group=1, s2_group=2, s3_group=1,
              maxpool_pos_stage1=None, maxpool_pos_stage2=None, maxpool_pos_stage3=None,
              maxpool_stage1_pos0_kernel=(2, 2), maxpool_stage1_pos0_stride=(2, 2),
...
if __name__ == "__main__":
    model1 = get_cp_resnet()
    model2 = get_cp_resnet_sd()

    input = torch.randn(1, 1, 256, 65)
    shape = input.size()
